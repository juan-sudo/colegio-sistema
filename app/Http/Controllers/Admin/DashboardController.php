...
class DashboardController extends Controller
{
    ...
    private function paymentsTrend(): array
    {
        ...
        $collected = DB::table('payments')
            ->select(DB::raw("DATE_FORMAT(paid_date, '%Y-%m') as ym"), DB::raw('SUM(paid) as total'))
        ...
        $pending = DB::table('payments')
            ->select(DB::raw("DATE_FORMAT(due_date, '%Y-%m') as ym"), DB::raw('SUM(amount - discount - paid) as total'))
            ->where('status', '!=', 'pagado')
