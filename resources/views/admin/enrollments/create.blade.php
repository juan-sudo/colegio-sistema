...
@section('content')
<h1 class="text-xl font-bold mb-4">Nueva matrícula</h1>
<form method="POST" action="{{ route('admin.enrollments.store') }}">
    ...
    <div class="bg-white rounded shadow p-4 grid gap-4 max-w-2xl">
        ...
        <div>
            <label class="block text-sm font-medium mb-1">Fecha matrícula</label>
            <input type="date" name="enrollment_date" value="{{ old('enrollment_date', date('Y-m-d')) }}" class="w-full border rounded p-2" required>
            @error('enrollment_date')<span class="text-red-600 text-sm">{{ $message }}</span>@enderror
        </div>
        <div>
            <label class="block text-sm font-medium mb-1">Estado</label>
            <select name="status" class="w-full border rounded p-2" required>
                <option value="matriculado" {{ old('status') == 'matriculado' ? 'selected' : '' }}>Matriculado</option>
                <option value="retirado" {{ old('status') == 'retirado' ? 'selected' : '' }}>Retirado</option>
                <option value="trasladado" {{ old('status') == 'trasladado' ? 'selected' : '' }}>Trasladado</option>
            </select>
