...
<form method="POST" action="{{ route('admin.academic.schedules.update', $schedule) }}">
    @csrf @method('PUT')
    <div class="bg-white rounded shadow p-4 grid gap-4 max-w-2xl">
        <div>
            ...
            <select name="grade_section_id" class="w-full border rounded p-2" required>
                ...
                <option value="{{ $gs->id }}" {{ old('grade_section_id', $schedule->grade_section_id) == $gs->id ? 'selected' : '' }}>{{ $gs->name }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="teacher_id" class="w-full border rounded p-2">
                ...
                <option value="{{ $teacher->id }}" {{ old('teacher_id', $schedule->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->fullName() }}</option>
                @endforeach
            </select>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="time" name="start_time" value="{{ old('start_time', $schedule->start_time) }}" class="w-full border rounded p-2" required>
            ...
            <div>
                ...
                <input type="time" name="end_time" value="{{ old('end_time', $schedule->end_time) }}" class="w-full border rounded p-2" required>
        ...
        <div>
            ...
            <input type="text" name="classroom" value="{{ old('classroom', $schedule->classroom) }}" class="w-full border rounded p-2">
